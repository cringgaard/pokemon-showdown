...
- `strategy.py` is downstream from the threat model. It continuously scores `GLACEON_FORTRESS`, `AGGRON_FORTRESS`, and `TACTICAL_OFFENSE`, applies the configured primary-plan margin, and then derives context-sensitive resource values with a nonlinear HP utility curve. Resource values never feed back into the win-condition calculation.
