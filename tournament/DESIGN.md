...
The tournament runtime is generated from the official `python:3.12.13-slim-bookworm` image. Preparation resolves and records its local immutable image ID, then builds a tournament-controlled runtime layer containing the harness `worker.py` and a non-root UID/GID `10001:10001`. Participant images use another tournament-generated Dockerfile, copy every regular submission file into `/submission`, and optionally run non-root `pip install --target /opt/participant-python -r /submission/requirements.txt`. Participant Dockerfiles and symlinks are rejected; apt, raw build flags, secrets, SSH forwarding, privileged entitlements, and arbitrary mounts are not supported. Build networking is available for pip dependency retrieval and is a separate dependency/supply-chain trust surface. `main.py` is not invoked by the tournament build definition.

Images are cached by a SHA-256 hash over the sandbox policy version, resolved tournament runtime image ID, relative participant paths, modes, sizes, and contents. Matches execute the resolved immutable participant image ID. The artifact records the participant content hash, base/runtime/participant image IDs, Python version, and effective sandbox policy.
...
Docker containers are a practical boundary for an internal competition, not a perfect hostile-kernel boundary. Docker daemon, Linux kernel, base-image, package-index, and dependency build compromise or escape are outside this milestone's threat model. VM/microVM isolation, signed dependency infrastructure, system packages, and GPU policy remain deferred.
